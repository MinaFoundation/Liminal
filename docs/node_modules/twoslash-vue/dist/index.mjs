import { defaultMapperFactory, createVueLanguagePlugin, resolveVueCompilerOptions, createLanguage, FileMap } from '@vue/language-core';
...
function createTwoslasher(createOptions = {}) {
  ...
  function getVueLanguage(compilerOptions, vueCompilerOptions) {
    ...
    function getLanguage() {
      const vueLanguagePlugin = createVueLanguagePlugin(ts, (id) => id, () => "", () => true, defaultCompilerOptions, resolveVueCompilerOptions(vueCompilerOptions));
  ...
  function twoslasher(code, extension, options = {}) {
    ...
    const compiled = fileCompiled.snapshot.getText(0, fileCompiled.snapshot.getLength()).replace(/(?=export const __VLS_globalTypesStart)/, "// ---cut-after---\n");
